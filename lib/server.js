...
PeerServer.prototype._initializeHTTP = function() {
  ...
  this._app.use(restify.CORS());
  this._app.use(restify.fullResponse());
...
PeerServer.prototype._startStreaming = function(res, key, id, token, open) {
  ...
  // Send List of all online peers
  var peers = Object.keys(this._clients.peerjs);
  
  if (open) {
    res.write(JSON.stringify({ type: 'OPEN', peers: peers }) + '\n');
  }
